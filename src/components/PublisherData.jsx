...
import { EditOutlined, SaveOutlined, PlusOutlined, SearchOutlined } from "@ant-design/icons";
...
const PublisherData = () => {
      ...
      if (newFilters[key]) {
        if (Array.isArray(newFilters[key])) {
          filtered = filtered.filter((item) => {
            ...
            return itemDate.isAfter(newFilters[key][0]) && itemDate.isBefore(newFilters[key][1]);
          });
        } else {
          filtered = filtered.filter((item) => item[key]?.toString().includes(newFilters[key]));
        }
      }
    });
  ...
  const columns = [
    ...Object.keys(data[0] || {}).filter((key) => !["id", "user_id", "key","created_at"].includes(key)).map((key) => ({
      title: key.replace(/([A-Z])/g, " $1").trim(),
      dataIndex: key,
      key,
      filterDropdown: () => (
        key.toLowerCase().includes("date") ? (
          <DatePicker onChange={(date, dateString) => handleFilterChange(dateString, key)} style={{ width: "100%" }} />
        ) : dropdownOptions[key] ? (
          <Select onChange={(value) => handleFilterChange(value, key)} style={{ width: "100%" }} allowClear>
            {dropdownOptions[key].map((option) => (
              <Option key={option} value={option} >{option}</Option>
            ))}
          </Select>
        ) : (
          <Input onChange={(e) => handleFilterChange(e.target.value, key)} placeholder={`Search ${key}`} />
        )
      ),
      ...
      render: (text, record) => (
        editingKey === record.id ? (
          dropdownOptions[key] ? (
            <Select value={editedRow[key]} onChange={(value) => handleChange(value, key)} style={{ width: "100%" }}>
              {dropdownOptions[key].map((option) => (
                <Option key={option} value={option}>{option}</Option>
              ))}
            </Select>
          ) : key.toLowerCase().includes("date") ? (
            <DatePicker value={editedRow[key] ? dayjs(editedRow[key]) : null} onChange={(date, dateString) => handleChange(dateString, key)} style={{ width: "100%" }} />
          ) : (
            <Input value={editedRow[key]} onChange={(e) => handleChange(e.target.value, key)} />
          )
        ) : (
          text
        )
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen flex flex-col items-center">
      <div className="w-full overflow-auto bg-white p-4 rounded shadow-md">
        <Button type="primary" icon={<PlusOutlined />} onClick={handleAddRow} className="mb-4">
          Add Row
        </Button>
        <Table columns={columns} dataSource={data} pagination={{ pageSize: 10 }} bordered loading={loading} />
      </div>
    </div>
  );
};
