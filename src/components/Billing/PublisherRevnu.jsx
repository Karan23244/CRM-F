...
export default function PublisherAccount() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.post(`${API}/billing/publisher-month-revenue`, {
        month,
        user_id: user?.id,
        assign_id: user?.assigned_subadmins || [],
      });
  ...
  const columnsRaw = [
    ...
    getColumnWithFilterAndPin("POC", "POC"),

    getColumnWithFilterAndPin("total_revenue", "Revenue", (v) => (
      <span
        style={{
          color: "#52c41a",
          fontWeight: 700,
